...
public class FacebookManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (!FB.IsLoggedIn)
        {
            print("facebook user is not logged in yet.");
    ...
    public void LogIn(bool IsClicked)
    {
        ...
        else
        {
            if (AutoLoginEnabled)
            {
                print("im here so autoLogin is on!");
                FB.LogInWithReadPermissions(fbPerms, OnLoggedIn);
            }
        }
    }
    ...
    {
        print("inside OnLoggedIn callback");
    ...
    private void OnUserInfoGrabbed(IResult _FbUserResp)
    {
        if (_FbUserResp.Error == null)
        {
            ...
            print("myFirstName is " + myFirstName);
            print("myLastName is " + myLastName);
            //StartCoroutine(ServerManager.Instance.LogIn(myFbId, myName, myEmail));
        }
        else
        {
